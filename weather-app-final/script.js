...
function getInput(){
    ...
    fetch(first_url).then((res)=>res.json()).then((maindata)=>{
        ...
        fetch(all_data_url).then((res)=>res.json()).then((data)=>{
            ...
            function dayDate(seconds)
            {
                ...
                return `${days[dateEle.getDay()]}`+" "+`${dateEle.getDate()}`+" "+`${months[dateEle.getMonth()]}`+" "+`${dateEle.getFullYear()}`;
            }
            ...
            document.getElementsByClassName("current-temp-value")[0].innerHTML = `${(data.current.temp -273).toPrecision(2)}`+"℃";
            document.getElementById("current-loc").innerHTML = `${toCap(input_user)}`;
            document.getElementsByClassName("description")[0].innerHTML = `${toCap(data.current.weather[0].description)}`; 
            document.getElementsByClassName("icon")[0].style.backgroundImage = `url(${iconUrl(data.current.weather[0].icon)})`;
            ...
            for (let i = 0; i < h_slider_time.length; i++) {
                ...
                h_slider_time[i].innerHTML = `${timeConvert(getDate(`${data.hourly[i].dt}`).toLocaleTimeString().substring(0,5))}`;
            ...
            for (let i = 0; i < h_description.length; i++) {
                h_description[i].innerHTML = `${toCap(data.hourly[i].weather[0].description)}`;
            ...
            for (let i = 0; i < h_icon.length; i++) {
                ...
                h_icon[i].style.backgroundImage = `url(${iconUrl(data.hourly[i].weather[0].icon)})`;

            }

            //hour feel like
            for (let i = 0; i < h_feel_like.length; i++) {
                h_feel_like[i].innerHTML=`${(data.hourly[i].feels_like - 273).toPrecision(2)}℃`;
            ...
            for (let i = 0; i < h_wind_direction.length; i++) {
                h_wind_direction[i].style.transform = `rotate(-${data.hourly[i].wind_deg}deg)`;
            ...
            for (let i = 0; i < d_day_temp.length; i++) {
                ...
                d_day_temp[i].innerHTML = `${(data.daily[i].temp.day - 273).toPrecision(2)}℃`;
            }

            for (let i = 0; i < d_night_temp.length; i++) {
                
                d_night_temp[i].innerHTML = `${(data.daily[i].temp.night - 273).toPrecision(2)}℃`;
            }
            
            for (let i = 0; i < d_icon.length; i++) {
                
                d_icon[i].style.backgroundImage = `url(${iconUrl(data.daily[i].weather[0].icon)})`;
            }

            for (let i = 0; i < d_description.length; i++) {
                
                d_description[i].innerHTML = `${toCap(data.daily[i].weather[0].description)}`;
            }

            for (let i = 0; i < d_min_max_temp.length; i++) {
                
                d_min_max_temp[i].innerHTML = `${(data.daily[i].temp.min - 273).toPrecision(2)}℃`+"/"+`${(data.daily[i].temp.max - 273).toPrecision(2)}℃`;
            }
